import requests
import logging
from config import EUROPEAN_MAKES
import streamlit as st

@st.cache_data(ttl=86400)
def get_makes():
    """
    Obtiene todas las marcas de la API de NHTSA y filtra solo las que están
    en la lista EUROPEAN_MAKES de config.py (insensible a mayúsculas/minúsculas).
    """
    try:
        url = "https://vpic.nhtsa.dot.gov/api/vehicles/GetAllMakes?format=json"
        res = requests.get(url, timeout=10)
        res.raise_for_status()
    except requests.RequestException as e:
        logging.error(f"Error al obtener marcas: {e}")
        return []

    try:
        data = res.json()
    except ValueError:
        logging.error(f"Respuesta no es JSON válida: {res.text[:200]}")
        return []

    if "Results" not in data:
        logging.error(f"Estructura inesperada: {data}")
        return []

    # Normalizamos a minúsculas para comparar
    makes_api = [m["Make_Name"].strip() for m in data["Results"] if "Make_Name" in m]
    makes_filtradas = sorted([
        m for m in makes_api
        if m.lower() in [em.lower() for em in EUROPEAN_MAKES]
    ])

    return makes_filtradas


@st.cache_data(ttl=86400)
def get_models(make: str):
    """Obtiene modelos de una marca concreta desde la API de NHTSA."""
    if not make:
        return []
    try:
        url = f"https://vpic.nhtsa.dot.gov/api/vehicles/GetModelsForMake/{make}?format=json"
        res = requests.get(url, timeout=10)
        res.raise_for_status()
    except requests.RequestException as e:
        logging.error(f"Error al obtener modelos: {e}")
        return []

    try:
        data = res.json()
    except ValueError:
        logging.error(f"Respuesta
